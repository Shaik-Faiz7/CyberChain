...
Classify the document based ONLY on OCR text and label it as mentioned below.
...
"""
        ...
        print("[WARN] Ollama classification failed:", e)
...
async def upload(username: str = Form(...), file: UploadFile = File(...)):
    ...
    "source": source, 
    "hash": file_hash,  # returns hash as well
    "debug": debug_info
    }
...
async def get_user_files(username: str):
    ...
    # ✅ Build hash history map ONCE (O(n) instead of O(n²))
    file_map = {}
    for entry in chain:
        if entry.get("user") == username:
            orig_name = entry.get("orig_filename", "")
            if orig_name:
                if orig_name not in file_map:
                    file_map[orig_name] = []
                file_map[orig_name].append({
                    "hash": entry.get("hash", ""),
                    "time": entry.get("time", ""),
                    "type": entry.get("type", "unknown"),
                    "confidence": entry.get("type_confidence", 0.0),
    ...
    out = []
    for fname in files:
        # Find metadata for this specific encrypted file
        meta = next((e for e in chain if e.get("user")==username and e.get("filename")==fname), {})
        orig_name = meta.get("orig_filename", fname.replace(".enc",""))
        
        # Get pre-built hash history (instant lookup)
        hash_history = file_map.get(orig_name, [])
